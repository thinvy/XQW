...
public:
    ...
    int Transmit(float *id_vector, int box[4], uint8_t camera_id, uint8_t person_id) {
        ...
        memcpy(result_frame.data.id_vector, id_vector, sizeof(result_frame.data.id_vector));
    ...
    int Receive(float *id_vector, int box[4], uint8_t & camera_id, uint8_t & person_id) {

        ResultFrameBD result_frame;
        int rx_result = receiver_->read(&result_frame, sizeof(result_frame));

        if ((result_frame._SOF_ != 0x66) || (result_frame._EOF_ != 0x88)) {
            std::cout << "rx_result" << rx_result << std::endl;
            return -1;
        }

        // frame = cv::Mat(cv::Size(TRANSMIT_MAT_WIDTH, TRANSMIT_MAT_HEIGTH), CV_8UC3);
        memcpy(id_vector, result_frame.data.id_vector, sizeof(result_frame.data.id_vector));
